...
export default function ScaffoldPanel({
  ...
  const handleConfirm = () => {
    ...
    const question = scaffoldData as Record<string, string>;
  ...
  const handleWatchVideo = () => {
    const explanation = scaffoldData as Record<string, string>;
  ...
  if (phase === 'scaffold1') {
    const question = scaffoldData as Record<string, unknown>;
    ...
    return (
      <div style={styles.card} className="card">
        <div style={styles.scaffoldHeader}>
          ...
          <span style={styles.conceptChip}>Isolating: {(question as Record<string, string>).concept}</span>
        </div>

        <div style={styles.questionText}>
          {parseLaTeX((question as Record<string, string>).question)}
        </div>
        ...
        <div style={styles.optionsList}>
          {Object.entries(question.options as Record<string, string>).map(([key, val]) => (
            <OptionButton
              ...
              isCorrect={(question as Record<string, string>).answer === key}
              isWrong={selectedOption === key && (question as Record<string, string>).answer !== key}
            ...
            {isCorrect ? (
              <div style={styles.successBlock} className="card">
                ...
                <p style={styles.bridgeText}>
                  <strong>Connection: </strong> {parseLaTeX((question as Record<string, string>).bridgeExplanation)}
                </p>
                <button
  ...
  if (phase === 'scaffold3') {
    const explanation = scaffoldData as Record<string, string>;
    const animation = (ANIMATIONS as Record<string, { file: string; title: string }>)[originalConcept] || null;
  ...
  if (phase === 'giveUp') {
    const solution = scaffoldData as Record<string, unknown>;
      ...
      <div style={styles.card} className="card">
        ...
        <div style={styles.solutionContainer}>
          ...
          <div style={styles.stepsList}>
            {(solution.solutionSteps as string[] | undefined)?.map((step: string, idx: number) => (
          ...
          <div style={styles.derivationBox}>
            <strong>Conclusion: </strong> {parseLaTeX(solution.finalDerivation as string)}
          </div>
        </div>
