...
function SafeMath({ latex }: SafeMathProps) {
...
interface FilteredGroup {
  subject: string;
  chapterName: string;
  formulas: Array<{ name: string; latex: string; desc: string }>;
}
...
export default function FormulaSheet() {
  ...
  const filteredData = useMemo((): FilteredGroup[] => {
    ...
    subjects.forEach((sub) => {
      ...
      const chaptersList = CHAPTERS[sub as keyof typeof CHAPTERS] || [];
      chaptersList.forEach((chapter: { id: string; name: string }) => {
        const chapterFormulas = (FORMULAS as Record<string, Array<{ name: string; latex: string; desc: string }>>)[chapter.id] || [];
        ...
        {isOpen && (
          <>
            ...
            <motion.div 
              ...
              style={styles.sheet as React.CSSProperties}
              className="glass"
            >
        ...
        <div style={styles.playerOverlay} onClick={() => setActiveAnim(null)}>
          <div style={styles.playerModal} onClick={(e) => e.stopPropagation()}>
            <AnimationPlayer
              ...
              description={`Visualization of ${activeAnim.title}`}
