...
export function scheduleReminder(hourOfDay: number) {
  localStorage.setItem('jeeforge_reminder_hour', String(hourOfDay));
...
export function fireAchievementUnlock(badgeName: string, badgeEmoji: string) {
  fireNotification(
...
export function fireDailyChallengeAlert(chapterName: string) {
  fireNotification(
