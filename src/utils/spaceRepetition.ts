...
export function getDueReviews(): ConceptLearned[] {
  const concepts = storage.getConceptsLearned();
  const now = new Date();
  return concepts.filter(c => new Date(c.nextReview) <= now);
}

export function markReviewed(concept: string, wasCorrect: boolean): void {
  const concepts = storage.getConceptsLearned();
...
interface ReviewStatus {
  status: 'overdue_critical' | 'due' | 'learned';
  label: string;
  badgeColor: string;
  badgeDimColor: string;
}

export function getConceptReviewStatus(conceptObj: ConceptLearned): ReviewStatus {
  const now = new Date();
