...
async function generateTestQuestionsBatch(subject: string, chapters: string[], count: number, difficulty: string = 'medium'): Promise<Question[]> {
...
export default function TestPage() {
  ...
  const [simType, setSimType] = useState<'pcm' | 'phy' | 'chem' | 'math'>('pcm');
  const [customSubject, setCustomSubject] = useState<'physics' | 'chemistry' | 'math'>('physics');
  const [selectedChapters, setSelectedChapters] = useState<Record<string, boolean>>(() => {
    const chapters = CHAPTERS['physics'] || [];
    const initial: Record<string, boolean> = {};
    chapters.forEach((ch: { id: string }) => { initial[ch.id] = true; });
    return initial;
  });
  ...
  const [weakTips, setWeakTips] = useState<Record<string, string>>({});
  const [tipsLoading, setTipsLoading] = useState(false);
  const [animatedScore, setAnimatedScore] = useState(0);

  const toggleChapter = (id: string) => setSelectedChapters(prev => ({ ...prev, [id]: !prev[id] }));
  ...
  const startTestLoading = async (subjectParamsList: SubjectParams[]) => {
    setTestPhase('loading');
    setLoadingProgress(0);
    const totalQs = subjectParamsList.reduce((acc: number, curr: SubjectParams) => acc + curr.count, 0);
    ...
    try {
      ...
      track('test_start', { type: configTab === 'sim' ? `sim_${simType}` : 'custom', subject: configTab === 'sim' ? simType : customSubject });
    } catch {
  ...
  const handleStartTest = () => {
    if (configTab === 'sim') {
      if (simType === 'pcm') {
        ...
        startTestLoading([{ subject: sub, chapters: CHAPTERS[sub as keyof typeof CHAPTERS].map(c => c.name), count: 25, difficulty: 'medium' }]);
      }
    } else {
      const active = CHAPTERS[customSubject]?.filter((c: { id: string }) => selectedChapters[c.id]).map((c: { name: string }) => c.name) || [];
  ...
  const handleSelectOption = (letter: string) => {
    setAnswers(prev => ({ ...prev, [currentQIdx]: letter }));
    track('test_question_answered', { isCorrect: letter === test?.questions[currentQIdx]?.answer });
  };
  ...
  async function handleSubmitTest() {
    if (!test) return;
    let correctCount = 0;
    const items: TestResult[] = test.questions.map((q: Question, idx: number) => {
    ...
    track('test_submit', { score: correctCount, total: test.questions.length });
    ...
    weeklyData.testHistory.push({ date: new Date().toISOString(), score: correctCount, total: test.questions.length, subject: isSimulation ? `Sim (${simType.toUpperCase()})` : `Custom (${customSubject.toUpperCase()})`, challengeMode: isSimulation } as (typeof weeklyData.testHistory)[number]);

    const wrongConcepts = [...new Set(items.filter((i: TestResult) => !i.correct).map((i: TestResult) => i.concept))];
...
{ "tips": { "conceptName": "one sentence action revision advice" } }`;
        ...
        setWeakTips((data.tips as Record<string, string>) || {});
      } catch { /* ignore */ }
      finally { setTipsLoading(false); }
    }
  }

  const sortedResults = useMemo(() => [...resultsList].sort((a: TestResult, b: TestResult) => (a.correct === b.correct ? 0 : a.correct ? 1 : -1)), [resultsList]);
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '10px' }}>
                {[
                  ...
                  <div
                    key={item.id}
                    onClick={() => setSimType(item.id as 'pcm' | 'phy' | 'chem' | 'math')}
                    style={{
                      padding: '12px 14px',
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button className="btn btn-ghost" style={{ padding: '0', fontSize: '11px' }} onClick={() => { const u: Record<string, boolean> = {}; CHAPTERS[customSubject]?.forEach((c: { id: string }) => u[c.id] = true); setSelectedChapters(u); }}>All</button>
                    <button className="btn btn-ghost" style={{ padding: '0', fontSize: '11px' }} onClick={() => { const u: Record<string, boolean> = {}; CHAPTERS[customSubject]?.forEach((c: { id: string }) => u[c.id] = false); setSelectedChapters(u); }}>None</button>
                  </div>
                </div>
                <div style={{ maxHeight: '160px', overflowY: 'auto', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', padding: '8px', backgroundColor: 'var(--bg-secondary)' }}>
                  {(CHAPTERS[customSubject] || []).map((ch: { id: string; name: string }) => (
      ...
      <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg-primary)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={styles.testHeader} className="glass">
          ...
          <div style={{ display: 'flex', gap: '4px', overflowX: 'auto', paddingTop: '4px' }}>
            {test.questions.map((_: Question, idx: number) => (
              <button
                key={idx}
        ...
        <div style={{ paddingTop: '100px', paddingBottom: '32px', flex: 1 }} className="mx-auto px-4 max-w-lg w-full">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '16px' }}>
            {Object.entries(question.options).map(([key, val]) => (
              <OptionButton key={key} letter={key} value={val} isSelected={answers[currentQIdx] === key} isCorrect={false} isWrong={false} showAnswer={false} disabled={false} onClick={() => handleSelectOption(key)} />
            ))}
          </div>
    ...
    return (
      <div style={{ minHeight: '100vh', paddingBottom: '48px' }}>
        <div className="mx-auto px-4 max-w-3xl" style={{ paddingTop: '64px' }}>
          ...
          {/* Weak Tips */}
          {resultsList.some((r: TestResult) => !r.correct) && (
          ...
          <div className="card" style={{ marginBottom: '16px' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {sortedResults.map((res: TestResult, i: number) => (
