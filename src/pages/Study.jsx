...
export default function Study() {
  ...
  const { gainXP, preferences, checkProgressionXP } = useUser();
  ...
  const [scaffoldL1Data, setScaffoldL1Data] = useState(null);
  const [scaffoldL2Data, setScaffoldL2Data] = useState(null);
  const [ladderData, setLadderData] = useState(null);
  const [workedSolutionData, setWorkedSolutionData] = useState(null);
  ...
  const [sessionHistory, setSessionHistory] = useState([]); // tracks { question, userAnswer, isCorrect, timeSpent, scaffoldUsed }
  ...
  const loadQuestion = useCallback(async (currentMood) => {
    ...
    try {
      ...
      const q = await generateQuestion(chapter, difficultyPoint, conceptsLearned, currentMood || mood);
  ...
  const handleGoToScaffoldPhase = async (targetPhase) => {
  ...
  const triggerSessionSummary = async () => {
    ...
    try {
      ...
      setScaffoldL1Data(sum); // cache summary data
      setPhase('summary');
    } catch (e) {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topHeader}>
        ...
        {phase !== 'mood' && phase !== 'summary' && (
          <div style={styles.progressDotsContainer}>
            {Array(preferences.questionsPerSession || 5).fill(0).map((_, idx) => {
          ...
          <AnimatePresence mode="wait">
            ...
            {phase === 'question' && currentQuestion && (
              ...
              >
                ...
                {xpFloater && (
                  <div style={styles.xpFloaterContainer}>
                      ...
                      <div style={styles.correctOverlay} className="card">
                        ...
                        {showWrongAccordion && (
                          <div style={styles.accordionBody}>
                            {Object.entries(currentQuestion.whyOthersWrong).map(([key, reason]) => (
            ...
            {phase === 'summary' && scaffoldL1Data && (
              ...
              >
                ...
                <div style={styles.aiSummarySection}>
                  ...
                  <p style={styles.summaryText}>{scaffoldL1Data.summary}</p>
                </div>

                <div style={styles.aiAdviceSection}>
                  <strong style={{ color: 'var(--warning)', fontSize: '13px' }}>Study Advice:</strong>
                  <p style={styles.summaryText}>{scaffoldL1Data.advice}</p>
                </div>
      ...
      <AnimatePresence>
        {showShortcutTooltip && (
          <>
            <div style={styles.drawerBackdrop} onClick={() => setShowShortcutTooltip(false)} />
...
const styles = {
  ...
  mainScrollable: {
    paddingTop: '80px',
    paddingBottom: '88px', // space for bottom tool tab
    flex: 1
  },
  moodAlign: {
    display: 'flex',
