...
import type { Chapter, Subject, Bookmark, ConceptLearned } from '../types';

const CHAPTER_LOOKUP: Record<string, { name: string; subject: string }> = {};
(Object.keys(CHAPTERS) as Array<keyof typeof CHAPTERS>).forEach(sub => {
  CHAPTERS[sub].forEach((ch: Chapter) => {
...
export default function StatsPage() {
  ...
  const [activeSubjectTab, setActiveSubjectTab] = useState<Subject>('physics');
  const [selectedMistakeCategory, setSelectedMistakeCategory] = useState('all');
  const [selectedConcept, setSelectedConcept] = useState<ConceptLearned | null>(null);
  ...
  const mistakeChartData = useMemo(() => {
    const counts: Record<string, number> = { conceptual_gap: 0, calculation_error: 0, misread_question: 0, distractor_trap: 0 };
  ...
  const MISTAKE_COLORS: Record<string, string> = { conceptual_gap: 'var(--danger)', calculation_error: 'var(--warning)', misread_question: 'var(--accent)', distractor_trap: '#a855f7' };
  ...
  const masteryChartData = useMemo(() => {
    return (CHAPTERS[activeSubjectTab] || []).map((ch: Chapter) => {
  ...
  const bookmarksGrouped = useMemo(() => {
    const groups: Record<string, { chapterName: string; subject: string; list: Bookmark[] }> = {};
    (bookmarks as Bookmark[]).forEach(b => {
              ...
              {filteredMistakes.map((m, i) => (
                <div key={i} style={{ padding: '10px', borderRadius: 'var(--radius-md)', backgroundColor: 'var(--bg-secondary)', fontSize: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{m.date ? new Date(m.date).toLocaleDateString() : ''}</span>
                  </div>
        ...
        <div className="card" style={{ marginBottom: '24px', padding: '20px' }}>
          ...
          {conceptsLearned.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingLeft: '12px', borderLeft: '1px solid var(--border-subtle)' }}>
              {Object.entries(
                conceptsLearned.reduce((acc: Record<string, ConceptLearned[]>, c) => {
                ...
                <div key={idx}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', paddingLeft: '12px' }}>
                    {list.map((c: ConceptLearned, cIdx: number) => {
        ...
        <div className="card" style={{ marginBottom: '24px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '3px', backgroundColor: 'var(--bg-secondary)', padding: '2px', borderRadius: 'var(--radius-sm)' }}>
              {(['physics', 'chemistry', 'math'] as Subject[]).map(sub => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {bookmarksGrouped.map((group, gIdx) => (
                <div key={gIdx}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {group.list.map((b: Bookmark, bIdx: number) => (
