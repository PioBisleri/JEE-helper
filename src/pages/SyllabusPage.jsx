...
export default function SyllabusPage() {
  ...
  const [sortBy, setSortBy] = useState('default'); // default, completion_desc, concept_count_desc

  const [expandedChapter, setExpandedChapter] = useState(null);
  const [loadingSummary, setLoadingSummary] = useState(false);
  const [summariesCache, setSummariesCache] = useState(() => storage.getChapterSummaries() || {});
  const [trigger, setTrigger] = useState(0); // For forcing re-renders on local storage changes
  ...
  // Helper to compile details of a single chapter
  const getChapterDetails = (chapter, subject) => {
    const progress = storage.getChapterProgress(chapter.id);
    const unlockedConcepts = progress.conceptsUnlocked || [];
    ...
    return {
      ...
      status,
      checkedSubtopics: progress.checkedSubtopics || [],
      stats
    };
  };

  // Compile all chapters and their progress details
  const allChaptersWithDetails = useMemo(() => {
    const _ = trigger; // Reference trigger to trigger recalculations and satisfy ESLint
    const compiled = [];
  ...
  // Filters and Sorts chapters grouped by subject
  const filteredChaptersBySubject = useMemo(() => {
    const groups = { physics: [], chemistry: [], math: [] };

    allChaptersWithDetails.forEach(ch => {
      // Text Search
      const matchesSearch = ch.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            ch.subtopics.some(s => s.toLowerCase().includes(searchQuery.toLowerCase())) ||
                            ch.difficulty_curve.some(c => c.toLowerCase().includes(searchQuery.toLowerCase()));
      
      // Status Filter
      const matchesStatus = statusFilter === 'all' || ch.status === statusFilter;

      if (matchesSearch && matchesStatus) {
        groups[ch.subject].push(ch);
      }
    });
    ...
    Object.keys(groups).forEach(sub => {
      if (sortBy === 'completion_desc') {
        groups[sub].sort((a, b) => b.completionPercentage - a.completionPercentage);
      } else if (sortBy === 'concept_count_desc') {
        groups[sub].sort((a, b) => b.totalConcepts - a.totalConcepts);
      }
      // 'default' keeps their syllabus index sequence
    });

    return groups;
  }, [allChaptersWithDetails, searchQuery, statusFilter, sortBy]);

  const handleToggleSubtopic = (chapterId, subtopic) => {
    const progress = storage.getChapterProgress(chapterId);
    const checked = progress.checkedSubtopics || [];
    let updated;
    if (checked.includes(subtopic)) {
      updated = checked.filter(s => s !== subtopic);
    } else {
      updated = [...checked, subtopic];
    }
    storage.updateChapterProgress(chapterId, { checkedSubtopics: updated });
    setTrigger(prev => prev + 1);
  };

  const handleFetchSummary = async (chapter) => {
    ...
    try {
      ...
      gainXP(20); // Reward for reading study guides
    } catch (err) {
      console.error(err);
        ...
        <div style={styles.statsCardGrid} className="grid">
          ...
          <div style={styles.subjectBreakdownCard} className="card">
            ...
            <div style={styles.subjectProgressList}>
              {Object.keys(aggregateMetrics.subjectProgress).map(sub => {
                                  ...
                                  <div style={styles.doubleColGrid}>
                                    ...
                                    <div style={styles.detailsCol}>
                                      ...
                                      <div style={styles.timeline}>
                                        {ch.difficulty_curve.map((concept, idx) => {
