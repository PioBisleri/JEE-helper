...
export default function DailyChallengePage() {
  ...
  const [completedChallengeData, setCompletedChallengeData] = useState<CompletedChallengeData | null>(null);
  ...
  const [userAnswers, setUserAnswers] = useState<UserAnswerEntry[]>([]); // array of { blueprint, question, chosen, correct, timeSpent }
  ...
  const calculateStreak = (history: ChallengeHistoryEntry[]) => {
    ...
    const dates = history.map((h: ChallengeHistoryEntry) => new Date(h.date).toDateString());
    const uniqueDates = [...new Set(dates)].map((d: string) => new Date(d));
  ...
  useEffect(() => {
    ...
    const todayAttempt = history.find((h: ChallengeHistoryEntry) => new Date(h.date).toDateString() === todayStr);

    if (todayAttempt) {
      setAlreadyCompletedToday(true);
      setCompletedChallengeData(todayAttempt as CompletedChallengeData);
  ...
  const generateBlueprints = () => {
    ...
    const getConceptScore = (concept: string) => {
      let score = 1;
      ...
      // Mistakes weight
      const mistakeCount = mistakes.filter(m => (m as Record<string, unknown>).concept === concept).length;
    ...
    const finalBlueprints: Blueprint[] = [];

    (Object.keys(distributions) as Array<keyof typeof CHAPTERS>).forEach(subject => {
      const subjectChapters = CHAPTERS[subject] || [];

      // Determine studied chapters
      let studiedChapterIds = subjectChapters
        .filter((ch: typeof subjectChapters[0]) => storage.getChapterProgress(ch.id).conceptsUnlocked.length > 0)
        .map((ch: typeof subjectChapters[0]) => ch.id);

      // Fallback if none studied: use first 2 chapters
      if (studiedChapterIds.length === 0) {
        studiedChapterIds = subjectChapters.slice(0, 2).map((ch: typeof subjectChapters[0]) => ch.id);
      }

      // Gather concepts in studied chapters with scores
      const candidateConcepts: Array<{ concept: string; chapterId: string; chapterName: string; score: number }> = [];
      subjectChapters.forEach((ch: typeof subjectChapters[0]) => {
        if (studiedChapterIds.includes(ch.id)) {
          ch.difficulty_curve.forEach((concept: string) => {
      ...
      candidateConcepts.sort((a: { score: number }, b: { score: number }) => b.score - a.score);

      // Select concepts for Easy, Medium, Hard slots
      const selectForDifficulty = (diffLabel: string, count: number) => {
      ...
      const subDist = (distributions as Record<string, Record<string, number>>)[subject];
  ...
  const loadQuestion = async (blueprint: Blueprint) => {
    ...
    try {
      ...
      setCurrentQuestion(q as unknown as GeneratedQuestion);
  ...
  const handleConfirmAnswer = () => {
    ...
    track('dc_question_answered', { isCorrect: correct });

    const timeSpent = Math.round((Date.now() - (questionStartTimeRef.current ?? Date.now())) / 1000);
    const blueprint = blueprints[currentIdx];

    // Log answer locally
    const answerLog: UserAnswerEntry = {
      blueprint,
      question: currentQuestion as unknown as Record<string, unknown>,
  ...
  const completeChallenge = () => {
    ...
    weeklyData.testHistory.push({
      ...
      challengeMode: true
    } as typeof weeklyData.testHistory[0]);
    ...
    const reportData: CompletedChallengeData = {
      date: new Date().toISOString(),
      score: correctCount,
      total: totalCount,
      timer: timer,
    ...
    setAlreadyCompletedToday(true);
    setGameState('summary');
    track('dc_complete', { score: correctCount, total: totalCount });
    showToast('Daily Challenge Completed! Graph data updated.', 'success');
  };

  // Compile summary statistics
  const summaryStats = useMemo(() => {
    const data = completedChallengeData || { date: '', score: 0, total: 30, timer: 0, answers: [] as UserAnswerEntry[] };
    ...
    // Subject breakdowns
    const subjects: Record<string, { attempted: number; correct: number }> = { physics: { attempted: 0, correct: 0 }, chemistry: { attempted: 0, correct: 0 }, math: { attempted: 0, correct: 0 } };
    const difficulties: Record<string, { attempted: number; correct: number }> = { easy: { attempted: 0, correct: 0 }, medium: { attempted: 0, correct: 0 }, hard: { attempted: 0, correct: 0 } };

    answers.forEach((ans: UserAnswerEntry) => {
    ...
    const getAccuracy = (obj: { attempted: number; correct: number }) => {
    ...
    return {
      ...
      wrongConcepts: answers.filter((a: UserAnswerEntry) => !a.correct).map((a: UserAnswerEntry) => a.blueprint.concept)
    };
  ...
  return (
    <div style={styles.page as React.CSSProperties}>
      <div className="mx-auto p-6 max-w-2xl" style={styles.content as React.CSSProperties}>

        {/* Landing Screen */}
        {gameState === 'landing' && (
          <div className="card text-center" style={styles.centerCard as React.CSSProperties}>
            ...
            <div style={styles.rulesList as React.CSSProperties}>
              <div style={styles.ruleItem as React.CSSProperties}>
            ...
            <div style={styles.streakBadgeLarge as React.CSSProperties} className="glass">
              Current Challenge Streak: <strong>{streakCount} Days</strong>
            </div>

            {alreadyCompletedToday ? (
              <div style={{ marginTop: '20px' }}>
                <div style={styles.cooldownAlert as React.CSSProperties} className="glass">
                  <span>You've completed today's challenge! Next lock unlocks in:</span>
                  <strong style={styles.cooldownCountdown as React.CSSProperties}>{timeLeftToMidnight}</strong>
                </div>
                <button
        ...
        {gameState === 'loading_blueprints' && (
          <div className="card text-center" style={styles.centerCard as React.CSSProperties}>
            <div style={styles.loaderSpinner as React.CSSProperties} />
        ...
        {/* Playing Screen */}
        {gameState === 'playing' && (
          <div style={styles.activeSessionWrapper as React.CSSProperties}>
            {/* Top Toolbar */}
            <div style={styles.sessionToolbar as React.CSSProperties} className="glass">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={styles.qIndexCounter as React.CSSProperties}>Question {currentIdx + 1} / 30</span>
                <span style={{
                  ...styles.diffLabel as React.CSSProperties,
              ...
              <div style={styles.sessionTimer as React.CSSProperties}>
            ...
            {loadingQuestion ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={styles.sessionQuestionCard as React.CSSProperties} className="card">
                  <div style={styles.chapTagsRow as React.CSSProperties}>
                    <span style={styles.subjectTag as React.CSSProperties}>{blueprints[currentIdx]?.subject.toUpperCase()}</span>
                    <span style={styles.chapterTag as React.CSSProperties}>{blueprints[currentIdx]?.chapterName}</span>
                  </div>
                  <p style={styles.questionText as React.CSSProperties}>
                ...
                <div style={styles.optionsList as React.CSSProperties}>
                  {Object.entries(currentQuestion.options).map(([key, val]) => {
                    ...
                    let btnStyle = { ...styles.optionBtn as React.CSSProperties };
                    if (isSelected) btnStyle = { ...btnStyle, ...styles.optionSelected as React.CSSProperties };
                    if (confirmed) {
                      if (isAns) btnStyle = { ...btnStyle, ...styles.optionCorrect as React.CSSProperties };
                      else if (isSelected) btnStyle = { ...btnStyle, ...styles.optionWrong as React.CSSProperties };
                    }
                    ...
                    return (
                      ...
                      >
                        <span style={{
                          ...styles.optionBadge as React.CSSProperties,
                          backgroundColor: isSelected ? 'var(--accent)' : 'var(--bg-elevated)'
                        }}>{key}</span>
                        <span style={styles.optionVal as React.CSSProperties}>{parseLaTeX(val)}</span>
                      </button>
                    );
                  })}
                </div>
                ...
                {/* Solution Reveal */}
                {confirmed && (
                  <div style={styles.solutionBox as React.CSSProperties} className="card">
                    <h5 style={{
                    ...
                    <p style={{ fontSize: '12px', color: 'var(--text-primary)', margin: '0 0 10px 0', lineHeight: '1.5' }}>
                      <strong>Concept:</strong> {currentQuestion.primaryConcept ?? currentQuestion.concept}
                    </p>
                ...
                {/* Bottom Navigation */}
                <div style={styles.sessionFooter as React.CSSProperties}>
                  <button
        ...
        {gameState === 'summary' && (
          <div className="card" style={{ padding: '24px' }}>
            ...
            <div style={styles.summaryTitleBlock as React.CSSProperties}>
            ...
            <div style={styles.scoreRow as React.CSSProperties}>
              <div style={styles.gradeCircle as React.CSSProperties}>
                <span style={styles.gradeVal as React.CSSProperties}>{summaryStats.grade}</span>
                <span style={styles.gradeLabel as React.CSSProperties}>Grade</span>
              </div>

              <div style={styles.scoreDetails as React.CSSProperties}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                  <span style={styles.scoreFraction as React.CSSProperties}>{summaryStats.score}</span>
                ...
                <div style={styles.xpBonusBadge as React.CSSProperties} className="glass">
                  +{summaryStats.score * 20} XP Earned
                </div>
              </div>
            </div>

            <hr style={styles.divider as React.CSSProperties} />

            {/* Subject Breakdowns */}
            <h4 style={styles.summarySecHeader as React.CSSProperties}>Subject Accuracy Breakdowns</h4>
            <div style={styles.accuracyGrid as React.CSSProperties}>
              <div style={styles.accCard as React.CSSProperties} className="card">
                <span style={styles.accLabel as React.CSSProperties}>PHYSICS</span>
                <span style={{ ...styles.accVal as React.CSSProperties, color: 'var(--accent)' }}>{summaryStats.subjects.physics}%</span>
              </div>
              <div style={styles.accCard as React.CSSProperties} className="card">
                <span style={styles.accLabel as React.CSSProperties}>CHEMISTRY</span>
                <span style={{ ...styles.accVal as React.CSSProperties, color: 'var(--success)' }}>{summaryStats.subjects.chemistry}%</span>
              </div>
              <div style={styles.accCard as React.CSSProperties} className="card">
                <span style={styles.accLabel as React.CSSProperties}>MATHS</span>
                <span style={{ ...styles.accVal as React.CSSProperties, color: 'var(--warning)' }}>{summaryStats.subjects.math}%</span>
              </div>
            </div>

            {/* Difficulty Accuracy */}
            <h4 style={{ ...styles.summarySecHeader as React.CSSProperties, marginTop: '20px' }}>Difficulty Performance</h4>
            <div style={styles.accuracyGrid as React.CSSProperties}>
              <div style={styles.accCard as React.CSSProperties} className="card">
                <span style={styles.accLabel as React.CSSProperties}>EASY</span>
                <span style={{ ...styles.accVal as React.CSSProperties, color: 'var(--success)' }}>{summaryStats.difficulties.easy}%</span>
              </div>
              <div style={styles.accCard as React.CSSProperties} className="card">
                <span style={styles.accLabel as React.CSSProperties}>MEDIUM</span>
                <span style={{ ...styles.accVal as React.CSSProperties, color: 'var(--warning)' }}>{summaryStats.difficulties.medium}%</span>
              </div>
              <div style={styles.accCard as React.CSSProperties} className="card">
                <span style={styles.accLabel as React.CSSProperties}>HARD</span>
                <span style={{ ...styles.accVal as React.CSSProperties, color: 'var(--danger)' }}>{summaryStats.difficulties.hard}%</span>
              </div>
            </div>

            {/* Mistakes logged */}
            {summaryStats.wrongConcepts.length > 0 && (
              <div style={styles.mistakesSection as React.CSSProperties}>
                <h4 style={{ ...styles.summarySecHeader as React.CSSProperties, margin: 0 }}>Weak Spot Concepts Flagged</h4>
                <p style={{ fontSize: '10px', color: 'var(--text-muted)', margin: '2px 0 10px 0' }}>
                  These have been automatically queued in your review deck.
                </p>
                <div style={styles.wrongConceptsChips as React.CSSProperties}>
                  {summaryStats.wrongConcepts.map((concept: string, idx: number) => (
                    <span key={idx} style={styles.wrongConceptChip as React.CSSProperties}>
                      {concept}
                    </span>
                  ))}
                </div>
            ...
            <div style={styles.cooldownSection as React.CSSProperties}>
              <div style={styles.cooldownAlert as React.CSSProperties} className="glass">
            ...
            <div style={styles.summaryFooterActions as React.CSSProperties}>
              <button
                className="btn btn-secondary"
                onClick={() => navigate('/')}
                style={{ flex: 1 }}
