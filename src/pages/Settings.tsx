...
export default function Settings() {
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    ...
    if (isAuthenticated) {
      syncManager.updateProfile({ name: localName, exam_date: localExamDate }).catch(() => {});
    }
  };

  const handlePrefChange = (key: string, val: unknown) => {
    updatePrefs({ [key]: val });
  };
  ...
  const handleAISave = () => {
    ...
    track('settings_ai_config_save');
    showToast('AI provider updated', 'success');
    // Sync to backend if authenticated
    if (isAuthenticated) {
      syncManager.updateAIConfig({
        provider: aiProvider,
        api_key: aiKey.trim(),
    ...
    <div style={styles.container} className="mx-auto p-6 max-w-3xl">
      ...
      <div style={styles.sectionsList}>
        ...
        <div style={styles.card} className="card">
          <h3 style={styles.sectionHeader}>Account</h3>
          {isAuthenticated ? (
            <>
              ...
              {user?.email && (
                <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: '0 0 12px' }}>{user.email}</p>
              )}
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: '0 0 12px' }}>
                Your data syncs to the cloud. Progress is backed up automatically.
              </p>
              <button 
                className="btn btn-ghost" 
                style={{ ...styles.flexBtn, color: 'var(--danger)' }}
                onClick={() => {
            ...
            <>
              <p style={styles.settingDesc}>
                Create an account to sync your progress across devices and back up your data.
              </p>
        ...
        {/* Change Password Card (only for authenticated users) */}
        {isAuthenticated && (
          <div style={styles.card} className="card">
            <h3 style={styles.sectionHeader}>Change Password</h3>
            <p style={styles.settingDesc}>Update your account password.</p>
            <form onSubmit={async (e) => {
              e.preventDefault();
              if (!currentPassword || !newPassword) { showToast('Fill in both fields', 'error'); return; }
              ...
              <input
                ...
                style={styles.input}
              />
              <button type="submit" disabled={passwordLoading} className="btn btn-secondary" style={styles.saveBtn}>
          ...
          {aiProvider && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px' }}>
              ...
              <input
                ...
                placeholder={(PROVIDERS as Record<string, Record<string, string>>)[aiProvider]?.placeholder || 'Enter key'}
                style={styles.input}
              />
              ...
              <input
                ...
                placeholder={(PROVIDERS as Record<string, Record<string, string>>)[aiProvider]?.model || 'e.g. gpt-4o'}
                style={styles.input}
              />
        ...
        <div style={styles.card} className="card">
          ...
          <div style={styles.rowSetting}>
            ...
            <select 
              value={(preferences.defaultMood as string) || ''} 
          ...
          <div style={styles.rowSetting}>
            ...
            <select 
              value={Number(preferences.questionsPerSession) || 5} 
          ...
          {preferences.notificationsEnabled !== false && (
            <>
              <div style={styles.rowSetting}>
                ...
                <input 
                  type="time" 
                  value={(preferences.reminderTime as string) || '19:00'} 
