...
export default function Onboarding() {
  ...
  const { updateName, updateExam, updatePrefs, isAuthenticated } = useUser();
  ...
  const handleNameSubmit = (e: React.FormEvent) => {
    ...
    if (localName.trim()) {
      ...
      if (isAuthenticated) {
        syncManager.updateProfile({ name: localName.trim() }).catch(() => {});
      }
      setStep(2);
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val) {
      const diff = Math.ceil((new Date(val).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
  ...
  const handleDateSubmit = (e: React.FormEvent) => {
    ...
    if (localExamDate) {
      ...
      // Sync to backend if authenticated
      if (isAuthenticated) {
        syncManager.updateProfile({ exam_date: localExamDate }).catch(() => {});
      }
      setStep(3);
  ...
  const handleAISkip = () => {
    setKeyError('');
    track('onboarding_ai_skipped');
    setStep(4);
  };

  const handleNotificationSetup = async (grant: boolean) => {
    if (grant) {
  ...
  const activeProvider = providerList.find(p => p.id === selectedProvider) || providerList[0];
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} style={styles.step}>
              ...
              {selectedProvider && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px' }}>
                  ...
                  <input
                    ...
                    placeholder={(PROVIDERS as Record<string, { model: string }>)[selectedProvider]?.model || 'e.g. gpt-4o'}
                    style={styles.input}
                  />
                </div>
              )}
              ...
              <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--border-subtle)' }}>
                <p style={{ fontSize: '12px', color: 'var(--text-muted)', textAlign: 'center', margin: '0 0 8px' }}>
                  You can add an API key later in Settings.
                </p>
                <button onClick={handleAISkip} className="btn btn-ghost" style={{ width: '100%' }}>
                  Skip for now
                </button>
              </div>
            </motion.div>
