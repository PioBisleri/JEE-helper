export const ACHIEVEMENTS = [
  ...
  { id: "perfect_week", title: "Perfect Score", icon: "TargetIcon", description: "Score 10/10 on a weekly test", check: (s) => s.weeklyTestHistory?.some(t => t.score === t.total) },
  { id: "test_champion", title: "Test Champion", icon: "TrophyIcon", description: "Score perfect on 5 different tests", check: (s) => { const perfects = s.weeklyTestHistory?.filter(t => t.score === t.total) || []; return perfects.length >= 5; } },
  { id: "score_80", title: "High Scorer", icon: "TargetIcon", description: "Score 80% or above on any test", check: (s) => s.weeklyTestHistory?.some(t => (t.score / t.total) >= 0.8) },
  { id: "score_90", title: "Excellence", icon: "TargetIcon", description: "Score 90% or above on any test", check: (s) => s.weeklyTestHistory?.some(t => (t.score / t.total) >= 0.9) },
  ...
  { id: "speed_10", title: "Quick Thinker", icon: "LightningIcon", description: "Solve 10 questions in under 30 seconds each", check: (s) => (s.speedSolves >= 10) },
  { id: "speed_50", title: "Lightning Fast", icon: "LightningIcon", description: "Solve 50 questions in under 30 seconds each", check: (s) => (s.speedSolves >= 50) },

  // ─── Daily Challenge ───
  { id: "daily_first", title: "Daily Starter", icon: "TargetIcon", description: "Complete your first daily challenge", check: (s) => (s.dailyChallengesCompleted >= 1) },
  { id: "daily_7", title: "Daily Devotee", icon: "TargetIcon", description: "Complete 7 daily challenges", check: (s) => (s.dailyChallengesCompleted >= 7) },
  { id: "daily_30", title: "Daily Warrior", icon: "TargetIcon", description: "Complete 30 daily challenges", check: (s) => (s.dailyChallengesCompleted >= 30) },
  { id: "daily_perfect", title: "Daily Perfect", icon: "TrophyIcon", description: "Score 30/30 on a daily challenge", check: (s) => (s.dailyChallengeBest >= 30) },
  { id: "daily_25plus", title: "Daily Ace", icon: "TargetIcon", description: "Score 25+ on a daily challenge", check: (s) => (s.dailyChallengeBest >= 25) },
  ...
  { id: "ladder_10", title: "Ladder Master", icon: "LadderIcon", description: "Complete 10 concept ladders", check: (s) => (s.laddersCompleted >= 10) },
  ...
  { id: "quiz_master", title: "Quiz Master", icon: "TrophyIcon", description: "Get 10/10 on 3 different tests", check: (s) => { const perfects = s.weeklyTestHistory?.filter(t => t.score === t.total) || []; return perfects.length >= 3; } },
  { id: "gate_crusher", title: "Gate Crusher", icon: "LockIcon", description: "Unlock a chapter via the gate challenge", check: (s) => (s.gateUnlocks >= 1) },
  { id: "gate_3", title: "Gate Breaker", icon: "LockIcon", description: "Unlock 3 chapters via gate challenges", check: (s) => (s.gateUnlocks >= 3) },
